...
DS_PORT=5076
DS_ADDR_LIST='172.21.72.16'

#echo python -m p4p.gw --server "$DS_ADDR_LIST:$DS_PORT" --cip "$US_ADDR_LIST" --cport $US_PORT --prefix gw: #python -m p4p.gw --server "$DS_ADDR_LIST:$DS_PORT" --cip "$US_ADDR_LIST" --cport $US_PORT --prefix gw:
echo python -m p4p.gw pscag1-cxi.conf
python -m p4p.gw pscag1-cxi.conf
